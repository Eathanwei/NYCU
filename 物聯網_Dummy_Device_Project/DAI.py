...
while True:
    try:
        ...
        if value1 != None:
            value[0]=value[1]
            value[1]=value[2]
            value[2]=int(value1[0])
            check=1 #如果check==1，代表連續三次的值誤差<=1
            if abs(value[0]-value[1])>1:
               check=0
            if abs(value[0]-value[2])>1:
               check=0
            if abs(value[1]-value[2])>1:
               check=0
            aver=int((value[0]+value[1]+value[2])/3)
            if check==1 and aver!=lastvalue: #連續三次的值誤差<=1，而且新平均值與上次平均值不同
               DAN.push ('Dummy_Sensor', [aver]) #傳送新平均值
               lastvalue=aver
               if aver>1: #如果新平均值>1，更新密碼
                  print('passwaord: ',aver)
