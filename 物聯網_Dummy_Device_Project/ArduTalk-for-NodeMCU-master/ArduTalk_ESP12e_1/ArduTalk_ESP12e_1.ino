...
void loop() {
    delay(1000);
    if (scale.is_ready()) {//每秒取得一次當前重量
      long reading = scale.read();
      data = round((reading+16380.0)/3240.0*7.5);
    ...
    if (millis() - cycleTimestamp > 200) {
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() == 51){//密碼錯誤，LED設為紅色
              analogWrite(16, 0);
              analogWrite(5, 1023);
              analogWrite(4, 1023);
            }
            if (result.toInt() == 102){//密碼正確，LED設為綠色
              analogWrite(16, 1023);
              analogWrite(5, 1023);
              analogWrite(4, 0);
            }
            if (result.toInt() != 51 && result.toInt() != 102){//待機或密碼更新，LED設為藍色
              analogWrite(16, 1023);
              analogWrite(5, 0);
              analogWrite(4, 1023);
            }
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(5, result.toInt()*4);
        }    

        result = pull("D2~");
        if (result != "___NULL_DATA___"){
            Serial.println ("D2~: "+result);
            if (result.toInt() >= 0 && result.toInt() <= 255) analogWrite(4, result.toInt()*4);
        }
