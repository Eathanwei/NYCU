...
correction = [np.array([0.0,0.0,0.0233], dtype='g'),np.array([0.0,0.0,-0.043], dtype='g'),np.array([0.0, 0.0, 0.0], dtype='g'),np.array([0.0, 0.0, 0.0], dtype='g'),np.array([0.0, 0.0, 0.0], dtype='g')]
...
async def main():
    ...
    await asyncio.sleep(60)  # Extend time as needed
...
def notification_handler(data, device_address):
    global last_timestamp, last_acceleration, last_velocity, acceleration, velocity, position, timestamp_slot, first_timstamp
    if len(data) >= 32:
        ...
        if last_timestamp[index] != 0:
            ...
            if index==0 or index==1:
                ...
                if timestampdiff-timestamp_slot[5]>0:
